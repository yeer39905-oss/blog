...
tags:
  ...
  - Neural Network
---
学习过MLP（多层感知机），CNN（卷积神经网络），RNN（循环神经网络）三个框架后，可以发现，三个模型本质上都是在做：输入数据X -> 特征提取 -> 分类器 ->  输出
不同的地方在于特征的提取的办法不同
接下来，我会用MPL模型做MNIST灰度手写图片分类，用CNN模型做CIFAR10图片分类，用RNN做情感分类为例，来说明三个模型的各自的用处和不同点
...
![MNIST灰度手写图片](/deeplearning/mlpMINIST.png)
把上面每一个epoch做的任务详细写：
1. 使用mini-bacth的方法，每次取64张MNIST图片作训练，每张图片展开成$784 \times 1$的列向量
...
$s_{dw}^{(t)} = \beta_2s_{dw}^{(t-1)} + (1-\beta_2)(dw^{(t)})^2$ 

用$dw$的平方消去正负影响和衡量梯度变化的剧烈程度

梯度$dw$除以$\sqrt{s_{dw}}$,
梯度大的方向学习率调小，小的方向学习率调大，就能达到自动调整学习率的效果，加上很小的数$\epsilon$ ,是为了防止分母为零
...
class MLP(nn.Module):#继承  
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(28*28,128)#MNIST图片展开成784为 输出128个神经元  
#self.fc1 = 一个全连接层  
...
### 与MLP比较，CNN更适合处理图像
刚刚提到，MLP是一开始就将数据展成一维向量，对于$28\times 28$ 这样的MNIST灰度手写图片或许还好，但是对于大尺寸的RGB图片，展开成一维向量之后参数量太大了 
...
图像边缘补0后，提取局部特征的同时保持空间尺寸不变,反复Conv -> ReLu -> Pool
...
然后引入词嵌入，当前我的例子中，词典库只有10个词，所以只需要10个
$10\times1$的one-hot向量就能表示所有词了，但是要是词典库有10 000或者100 000个词呢？

如果还用这样的方法表示，参数量就会很大，而且不能表示出词与词之间语义联系
